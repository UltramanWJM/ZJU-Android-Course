...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator SizeX = ObjectAnimator.ofFloat(target,
                "scaleX",
                1f,
                2f);
        SizeX.setRepeatCount(ValueAnimator.INFINITE);
        SizeX.setInterpolator(new LinearInterpolator());
        SizeX.setDuration(Integer.parseInt(durationSelector.getText().toString()));

        ObjectAnimator SizeY = ObjectAnimator.ofFloat(target,
                "scaleY",
                1f,
                2f);
        SizeY.setRepeatCount(ValueAnimator.INFINITE);
        SizeY.setInterpolator(new LinearInterpolator());
        SizeY.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        // TODO ex2-2：在这里实现另一个 ObjectAnimator，对 target 控件的透明度进行修改，从 1 到 0.5f 循环
        ObjectAnimator tran = ObjectAnimator.ofFloat(target,
                "alpha",
                1f,0.5f);
        tran.setRepeatCount(ValueAnimator.INFINITE);
        tran.setDuration(Integer.parseInt(durationSelector.getText().toString()));

        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1,SizeX,SizeY,tran);
        animatorSet.start();
    }
}
